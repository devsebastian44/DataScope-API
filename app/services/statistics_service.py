...
class StatisticsService:
    ...
    @staticmethod
    def calculate_statistics(dataset_id: str, columns: Optional[List[str]] = None,
        ...
        for col in df_analysis.columns:
            stats_dict = StatisticsService._calculate_column_statistics(df_analysis, col)
        ...
        return {
            ...
            "memory_usage_mb": df_analysis.memory_usage(deep=True).sum() / (1024 * 1024),
    ...
    def _calculate_column_statistics(df: pd.DataFrame, column: str) -> dict:
        ...
        # Estadísticas categóricas
            ...
            if len(mode_val) > 0:
                ...
                stats_dict["mode_frequency"] = int(series.value_counts().iloc[0])
            
            # Top 5 valores más frecuentes
            top_values = series.value_counts().head(5).to_dict()
            stats_dict["top_values"] = {str(k): int(v) for k, v in top_values.items()}
        
        return stats_dict
    
    @staticmethod
    def calculate_distribution(dataset_id: str, column: str, bins: int = 30) -> dict:
        ...
        if pd.api.types.is_numeric_dtype(series):
            ...
            for i in range(len(counts)):
                ...
                distribution[bin_label] = int(counts[i])
        ...
        # Para columnas categóricas
            ...
            result["distribution"] = {str(k): int(v) for k, v in value_counts.items()}
